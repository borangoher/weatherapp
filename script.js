...
const displayData = async function (weatherJSON) {
  const weatherObject = await weatherJSON.then(function (response) {
    return response;
  });
  ...
  weatherType.innerHTML =
    ...
    `
                            <small class="text-muted" id="desc">` +
    weatherObject.weather[0].description +
    `</small>`;
  tempVal.textContent =
    "Temperature: " + toCelcStr(weatherObject.main.temp) + "°C";
  tempApp.textContent =
    "Apparent Temperature: " + toCelcStr(weatherObject.main.feels_like) + "°C";
  tempMax.textContent =
    "Maximum Temperature: " + toCelcStr(weatherObject.main.temp_max) + "°C";
  tempMin.textContent =
    "Minimum Temperature: " + toCelcStr(weatherObject.main.temp_min) + "°C";
  humVal.textContent =
    "Humidity: " + weatherObject.main.humidity.toFixed(2) + "%";
  presVal.textContent =
    "Pressure: " + weatherObject.main.pressure.toFixed(2) + "mb";
};

formElem.addEventListener("submit", function (event) {
  event.preventDefault();
  displayData(fetchData(createURL(searchBar, APIKey)));
});
